...
class RdfDatasetTests(unittest.TestCase):
    ...
    def test_fingerprint_is_stable_across_independent_fresh_assemblies(self) -> None:
        first = MODULE.assemble_dataset()
        second = MODULE.assemble_dataset()
    ...
    def test_fingerprint_preserves_named_graph_identity(self) -> None:
        ...
        obj = Literal("value")
        left.graph(URIRef("https://w3id.org/project-chemie-digital/graph/a")).add((subject, predicate, obj))
        right.graph(URIRef("https://w3id.org/project-chemie-digital/graph/b")).add((subject, predicate, obj))
    ...
    def test_fingerprint_ignores_parser_local_blank_node_identifiers(self) -> None:
        ...
        graph_id = URIRef("https://w3id.org/project-chemie-digital/graph/shapes/example")
        predicate = URIRef("https://example.invalid/p")
        first.graph(graph_id).add((BNode("first"), predicate, Literal("value")))
        second.graph(graph_id).add((BNode("second"), predicate, Literal("value")))
        self.assertEqual(MODULE.dataset_fingerprint(first), MODULE.dataset_fingerprint(second))

    def test_canonical_nquads_preserves_literal_metadata_and_control_escapes(self) -> None:
        dataset = Dataset(default_union=False)
        graph_id = URIRef("https://w3id.org/project-chemie-digital/graph/test")
        subject = URIRef("https://example.invalid/s")
        predicate = URIRef("https://example.invalid/p")
        graph = dataset.graph(graph_id)
        graph.add((subject, predicate, Literal('line 1\nline 2 "quoted"\\tail\tend', lang="en")))
        graph.add((subject, URIRef("https://example.invalid/n"), Literal(3)))
        rendered = MODULE.canonical_nquads(dataset)
        self.assertIn('"line 1\\nline 2 \\"quoted\\"\\\\tail\\tend"@en', rendered)
        self.assertIn('"3"^^<http://www.w3.org/2001/XMLSchema#integer>', rendered)
        reparsed = Dataset(default_union=False)
        reparsed.parse(data=rendered, format="nquads")
        self.assertEqual(2, len(reparsed.graph(graph_id)))

    def test_blank_node_identity_outside_shapes_is_rejected(self) -> None:
        dataset = Dataset(default_union=False)
        graph = dataset.graph(URIRef("https://w3id.org/project-chemie-digital/graph/specifications/test"))
        graph.add((BNode(), RDF.type, URIRef("https://example.invalid/T")))
        with self.assertRaisesRegex(ValueError, "Blank-node subjects are permitted only inside SHACL graphs"):
            MODULE.validate_dataset_contract(dataset)
