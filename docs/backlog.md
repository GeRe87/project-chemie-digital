...
40. Implement ADR-0009 in the active offline canonical-runtime path: add a renderer-neutral course/unit/path selector over the immutable assembled RDF Dataset, delegate one validated selected path explicitly to scene compilation, remove the global single-LearningPath discovery assumption, and cover singleton, explicit multi-path, ambiguity and membership failures without changing canonical RDF, renderer contracts or learner-state.
41. Record the renderer-neutral teaching-offering runtime/read-model boundary before implementing multi-unit application navigation: define a versioned disposable runtime document derived from validated `TeachingOffering` composition that preserves stable offering/placement/unit/path identities, authored display metadata and deterministic course order while explicitly excluding URLs/routes, current selection, learner progress, renderer mechanics and LMS state.
42. Implement ADR-0010 in the active offline canonical-runtime generator: add a deterministic `TeachingOfferingRuntimeDocument 1.0` projector over the immutable assembled RDF Dataset and package `teachingOfferingDocuments[]` additively into the existing `artifactVersion: "1.0"` container, preserving exact offering/placement/unit/path identities, authored multilingual display metadata, course ordering, path graph provenance and the root Dataset fingerprint without adding application navigation, current selection, learner-state, semantic-client or ontology changes.
43. Establish the shared TypeScript consumption boundary for the generated course runtime before implementing navigation: add framework-neutral `TeachingOfferingRuntimeDocument 1.0` and canonical-runtime artifact types/validators in `packages/core`, reject mixed Dataset fingerprints and malformed normalized course structure, and migrate the self-study browser/static loaders to that shared validator while exposing course documents read-only and adding no menu, routing, current selection, path-choice UX or authored semantic changes.
