...
const autoAnimateSceneIds = new Set([
  "ex:scene-cogniflow-fair-processing-gap--scene",
  "ex:scene-cogniflow-explicit-processing-context--scene",
]);
const COGNIFLOW_NETWORK_AUTO_ANIMATE_ID = "cogniflow-network-comparison";
...
function appendBlock(parent: MinimalElement, dom: PitchDomPort, block: SceneBlock, headingId: string): void {
  ...
  if (block.kind === "media-reference") {
    ...
    if (block.source.some((source) =>
      source.resourceId === "ex:media-cogniflow-laboratory-chaos"
      || source.resourceId === "ex:media-cogniflow-shared-fair-platform"
    )) {
      figure.setAttribute("data-id", "cogniflow-network-visual");
    }
...
export function mountSceneDocuments(dom: PitchDomPort, documents: readonly SceneDocument[]): () => void {
  ...
  for (const document of documents) for (const scene of document.scenes) {
    ...
    if (autoAnimateSceneIds.has(scene.id)) {
      section.setAttribute("data-auto-animate", "");
      section.setAttribute("data-auto-animate-id", COGNIFLOW_NETWORK_AUTO_ANIMATE_ID);
      section.setAttribute("data-auto-animate-duration", "0.18");
      section.setAttribute("data-auto-animate-easing", "linear");
    }
