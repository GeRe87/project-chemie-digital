...
- Snapshot entity labels now prefer `skos:prefLabel` in the effective selected-path language, followed only by deterministic authored fallbacks (`skos:prefLabel`, `dct:title`, `schema:name`, then local identity).
- Snapshot entity descriptions now prefer `cd:body` in the effective language, then `dct:description` in that language, followed only by deterministic authored-language fallback from those same predicates. No translation or generated text is introduced.
...
- Missing requested language on one resource uses a deterministic authored fallback and remains stable across repeated snapshot generation.
...
This connector-oriented worker cannot execute the repository-local `npm test` command itself. The focused regressions were authored for the repository validator, and fresh exact-head `agent-validator/project-chemie-digital` success remains mandatory before manager acceptance. The Draft PR includes the configured validator marker so the local validator can publish authoritative exact-head evidence.
...
2. Verify the PR remains limited to generator/runtime summary projection, focused tests, this handoff, and System workflow state.
