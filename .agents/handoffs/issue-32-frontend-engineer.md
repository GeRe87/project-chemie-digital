...
- Corrected configuration validation so every entry, detail and return component in a detail path must belong to one canonical section; cross-section paths now fail before any presenter document is produced.
- Added a navigation guard preventing a valid detail path from being entered while another canonical section is active.
...
- Added deterministic, privacy-separation, optional-path, canonical-order, focus-restoration, invalid-input, cross-section, lifecycle and no-network tests.
...
- [x] Cross-section entry, detail and return references fail atomically without a partial presenter document.
- [x] Entering a valid path from the wrong current canonical section fails deterministically.
- [x] Keyboard focus restoration, timer state, reduced motion, idempotent cleanup and no-network behavior are covered.
- [ ] Exact-head local validation must report `agent-validator/project-chemie-digital: success` for the corrected final PR head.
...
- A detail path is section-local: its entry, every detail component and its return target must be contained in one canonical section.
...
- Focus is restored to the configured canonical return component in the same section.
...
Reload draft PR #33, require `agent-validator/project-chemie-digital` success on its corrected exact current head, verify the section-local detail-path validation and regression tests, and accept or request changes without using GitHub Actions evidence.
