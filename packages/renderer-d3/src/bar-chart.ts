...
export interface D3BarChartRenderModelResult {
  readonly model?: D3BarChartRenderModel;
  readonly diagnostics: readonly D3BarChartDiagnostic[];
}
export interface D3BarChartComponent {
  readonly model: D3BarChartRenderModel;
  readonly staticFallback: string;
...
function fallback(block: Extract<ChartBlock, { chartType: "bar" }>): string {
...
export function createD3BarChartRenderModel(block: ChartBlock): D3BarChartRenderModelResult {
  if (block.kind !== "chart" || block.chartType !== "bar") {
    return invalid("renderer-d3 bar charts require a canonical bar ChartBlock");
  }
...
export function mountD3BarChart(
  host: unknown,
  block: ChartBlock,
  options: D3BarChartOptions,
): D3BarChartComponent | D3BarChartRenderModelResult {
  const result = createD3BarChartRenderModel(block);
  if (!result.model) return result;
  if (!(host instanceof HTMLElement)) {
    return {
      diagnostics: [{ code: "INVALID_BAR_CHART_HOST", message: "D3 bar chart host must be an HTMLElement" }],
    };
  }
  ...
  const render = (): void => {
    ...
    const axis = svg("g");
    axis.classList.add("d3-chart-axis-layer");
    const bars = svg("g");
    bars.classList.add("d3-chart-bar-layer");
    const labels = svg("g");
    labels.classList.add("d3-chart-label-layer");

    for (const tick of y.ticks(5)) {
      const yy = y(tick);
      const line = svg("line");
      line.classList.add("d3-chart-grid-line");
    ...
    model.data.forEach((datum, index) => {
      const bx = x(datum.id) ?? margin.left;
      const by = y(datum.value);
      const bw = x.bandwidth();
      const bh = Math.max(0, margin.top + plotHeight - by);
      const rect = svg("rect");
      rect.classList.add("d3-chart-bar", `d3-chart-series-${(index % 5) + 1}`);
      rect.setAttribute("data-datum-id", datum.id);
      rect.dataset.value = String(datum.value);
      ...
      rect.style.transition = options.reducedMotion ? "none" : "transform 520ms cubic-bezier(.2,.8,.2,1), opacity 320ms ease, filter 320ms ease";
      bars.append(rect);
      ...
      valueText.textContent = String(datum.value);
      valueText.style.transition = options.reducedMotion ? "none" : "opacity 320ms ease, filter 320ms ease";
      labels.append(valueText);

      const category = svg("text");
      category.classList.add("d3-chart-category-label");
      category.setAttribute("x", String(bx + bw / 2));
      category.setAttribute("y", String(margin.top + plotHeight + 32));
      category.setAttribute("text-anchor", "middle");
      wrapLabel(datum.category).forEach((line, lineIndex) => {
        const tspan = svg("tspan");
        tspan.setAttribute("x", String(bx + bw / 2));
        tspan.setAttribute("dy", lineIndex === 0 ? "0" : "1.15em");
        tspan.textContent = line;
        category.append(tspan);
      });
    ...
    xTitle.textContent = model.xAxisLabel;
    labels.append(xTitle);
    const yTitle = svg("text");
    yTitle.classList.add("d3-chart-axis-title");
    yTitle.setAttribute("transform", `translate(24 ${margin.top + plotHeight / 2}) rotate(-90)`);
  ...
  render();
  const resizeObserver = typeof ResizeObserver !== "undefined" ? new ResizeObserver(() => render()) : undefined;
